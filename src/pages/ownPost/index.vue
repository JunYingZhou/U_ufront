        ...
        <view v-if="activeTab === 'posts'">
            <view class="post-list">
                <view class="post-item" v-for="post in posts" :key="post.id">
                    <view class="operation" @click.stop="showOperation(post.id)">
                        ...
                        <!-- 添加操作菜单 -->
                        <view v-if="showOperationIndex === post.id" class="operation-menu">
                            <text class="menu-item" @click.stop="handleModify(post)">修改</text>
                            <text class="menu-item" @click.stop="handleDelete(post)">删除</text>
                        </view>
                    </view>
...
const showOperationIndex = ref<number | null>(null)

const showOperation = (index: number) => {
    showOperationIndex.value = showOperationIndex.value === index ? null : index
}
...
const init = async() => {
   ...
   let res: any = await Promise.all([ getArticleByUserId(userId),getQuestionByUserId(userId)])
   console.log('个人发布详细数据',res) 
   if(res[0].msg == 'ok' && res[1].msg == 'ok') {
