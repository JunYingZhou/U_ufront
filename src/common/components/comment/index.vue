<template>
  <view class="hb-comment" v-if="loading">
    ...
    item.commentLikeCount == 0 ? "抢首赞" : item.commentLikeCount
  }}</span>
    ...
    item.commentContent.length > 60
      ? item.commentContent.slice(0, 59)
      : item.commentContent
  }}
    ...
    each.commentLikeCount == 0
      ? "抢首赞"
      : each.commentLikeCount
  }}</span>
    ...
    each.commentContent.length > 60
      ? each.commentContent.slice(0, 59)
      : each.commentContent
  }}
    ...
    <view class="comment-submit-box" v-if="submit" @click="closeInput">
      ...
      <view class="comment-add" @click.stop.prevent="stopPrevent" :style="'bottom:' + KeyboardHeight + 'px'">
        ...
        <textarea class="textarea" v-model="commentReq.content" :placeholder="placeholder" :adjust-position="false"
          :show-confirm-bar="false" @blur="blur" @focus="focusOn" :focus="focus" maxlength="800"></textarea>
      </view>
    </view>
    <!-- 新增评论-end -->
  </view>
...
const emptyAvatar = ref(
  "http://117.72.78.239:9000/zjyminio/defaultAvatar.webp"
);
...
function setHasLike(comments, personalList) {
    comments.forEach((item) => {
        // 检查当前评论是否在个人列表中
        for (const element of personalList) {
        ...
        if (item?.children && item.children.length > 0) {
            setHasLike(item.children, personalList);
        }
    });
}
...
//   // 对res[1]进行去重
//   if (res[1] && res[1].msg == 'ok') {
//     const userIdTemp = userStore.getUserId;
//     personalList = res[1].data.filter((item) => {
//       if (item.userId === userIdTemp) {
//         return true;
//       }
...
//     res[0].data.comment = getTree(res[0].data.comment);
//     commentData.value = res[0].data;
//     commentData.value.comment.forEach((item) => {
//       if (item?.children && item.children.length > 0) {
